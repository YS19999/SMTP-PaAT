...
def train(train_data, val_data, model, args):
    ...
    '''
    record = open('record.txt', 'a', encoding='utf-8')
    # creating a tmp directory to save the models
    out_dir = os.path.abspath(os.path.join(
                                      os.path.curdir,
                                      "temp-runs",
                                      args.embedding + '_' + args.classifier + '_' + args.dataset + '_' + str(args.way) + '_' + str(args.shot)))
    ...
    for ep in range(args.train_epochs):
        ...
        for task in sampled_tasks:
            if task is None:
                break
            loss = train_one(task, model, opt, args, grad)
        ...
        cur_acc, cur_std = test(val_data, model, args, args.val_episodes, False,
                                val_gen.get_epoch())
        record.writelines(str(loss) + '\t' + str(cur_acc) + '\n')
...
def train_one(task, model, opt, args, grad):
    ...
    _, loss = model['clf'](support, query)
