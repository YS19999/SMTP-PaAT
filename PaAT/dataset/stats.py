...
def get_w_target_rr(data, text_ebd, w_target_lam, flag='support'):
    '''
    ...
    label_onehot = F.embedding(label, torch.eye(len(unique), dtype=torch.float, device=text_ebd.device))

    I = torch.eye(len(text_ebd), dtype=torch.float, device=text_ebd.device)

    # w = text_ebd.t() @ torch.inverse(text_ebd @ text_ebd.t() + w_target_lam * I)
    w = text_ebd.t() @ torch.inverse(text_ebd @ text_ebd.t() + w_target_lam * I) @ label_onehot
    return w


def get_w_target(data, text_ebd, w_target_lam):
    '''
        Compute the importance of every tokens in the support set
